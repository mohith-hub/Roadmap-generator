...
export default function CreateRoadmap() {
  ...
  const [saveRoadmapName, setSaveRoadmapName] = useState("");
  ...
  const handleConfirmSave = () => {
    if (saveRoadmapName.trim() && (userData.links.length > 0 || userData.files.length > 0)) {
      const roadmapToSave = { originalRoadmap: roadmap || [], userLinks: userData.links, userFiles: userData.files };
  ...
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#FBE4D6] p-4">
        <h1 className="text-2xl md:text-3xl font-bold text-[#161179] mb-4 md:mb-6">Create Your Roadmap</h1>
        <select
